...
public:
    ...
    // Construction
    SGDFunc(const float lrValue) : lr(lrValue) {}
...
public:
    ...
    // Construction to initialize the constant b
    __host__ __device__ AddConstFunc(const T& value) : b(value) {}
    const T b;
};
...
public:
    ...
    // Construction
    __host__ __device__ MultiplyConstFunc(const T& bValue) : b(bValue) {}
...
__global__ void op_elemwise_unary_kernel(OpFunc f, Tensor<T> t, Tensor<T> out)
{
    ...
    if (row < t.h && col < t.w) {
        T elem = Index(t, row, col);
        T result = f(elem);
        Index(out, row, col) = result;
    }
}
...
void op_elemwise_unary_gpu(OpFunc f, const Tensor<T> &t, Tensor<T> &out)
{
    ...
    //assert(0);
    assert(t.h == out.h && t.w == out.w);
    // Calculate the number of blocks and threads per block.
    dim3 dimBlock(ELEMWISE_BLOCK_DIM, ELEMWISE_BLOCK_DIM);
    dim3 dimGrid((t.w + dimBlock.x - 1) / dimBlock.x, (t.h + dimBlock.y - 1) / dimBlock.y);

    op_elemwise_unary_kernel<<<dimGrid, dimBlock>>>(f, t, out);
...
__global__ void op_elemwise_binary_w_bcast_kernel(OpFunc f, Tensor<AT> in1, Tensor<BT> in2, Tensor<OutT> out)
{
    ...
    if (row < in1.h && col < in1.w) {
        int in2_row = (in2.h == 1)? 0 : row;
        int in2_col = (in2.w == 1)? 0 : col;

        out.rawp[out.offset + row * out.stride_h + col * out.stride_w] = 
        f(in1.rawp[in1.offset + row * in1.stride_h + col * in1.stride_w],
          in2.rawp[in2.offset + in2_row * in2.stride_h + in2_col * in2.stride_w]);
...
void op_elemwise_binary_w_bcast_gpu(OpFunc f, const Tensor<AT> &in1, const Tensor<BT> &in2, Tensor<OutT> &out)
{
    ...
    dim3 gridDim((in1.w + blockDim.x - 1) / blockDim.x, (in1.h + blockDim.y - 1) / blockDim.y);
